...
function activate(context) {
  ...
  function updateDecorations() {
    try {
      ...
      util.getAllMacro().forEach((macro) => {
        context[macro] = true;
      });
  ...
  function updateFolding() {
    ...
    let foldingRangeProvider = {
      provideFoldingRanges() {
        ...
        foldingRangeArr = foldingRangeArr.concat(
          util.getFoldingRangeList(document, htmlRule)
        );
        foldingRangeArr = foldingRangeArr.concat(
          util.getFoldingRangeList(document, scriptOrStyleRule)
        );
        return foldingRangeArr;
      },
    };
    folding = vscode.languages.registerFoldingRangeProvider(
      { pattern: "**", scheme: "file" },
